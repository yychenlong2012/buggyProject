...
{   
    ...
    self.bgView.backgroundColor = [self colorWithHexString:@"#EEEEEE"];
...
- (void)layoutSubviews
{
    ...
    //图片
    _rightImageView.frame = CGRectMake((self.bounds.size.width-diameter)/2,
                                           (titleHeight-diameter)/2,
                                           diameter,
                                           diameter);
    //背景
    self.bgView.frame = CGRectMake((self.bounds.size.width-diameter)/2,
                                   (titleHeight-diameter)/2,
                                   diameter,
                                   diameter);
...
- (void)configureAppearance
{
    ...
    if (!shouldHideShapeLayer) {
        ...
        self.bgView.layer.cornerRadius = CGRectGetWidth(_shapeLayer.bounds)*0.5; //自己添加
        CGPathRef path = [UIBezierPath bezierPathWithRoundedRect:_shapeLayer.bounds
                                                    cornerRadius:CGRectGetWidth(_shapeLayer.bounds)*0.5*self.borderRadius].CGPath;
        if (!CGPathEqualToPath(_shapeLayer.path, path)) {
            _shapeLayer.path = path;
