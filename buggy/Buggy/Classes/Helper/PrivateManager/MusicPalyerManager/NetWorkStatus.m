...
//    NSArray *subviews = [[[[UIApplication sharedApplication] valueForKey:@"statusBar"] valueForKey:@"foregroundView"] subviews];
    ...
    NSNumber *num = [dataNetworkItemView valueForKey:@"dataNetworkType"];      // 3 : 4G 、 5：wifi
    if (num == nil) {
    ...
    //适配iPhone X
    if ([MainModel model].networkStatus == AFNetworkReachabilityStatusNotReachable) {
        networkType = NETWORK_TYPE_NONE;
    }
    if ([MainModel model].networkStatus == AFNetworkReachabilityStatusReachableViaWiFi) {
        networkType = NETWORK_TYPE_WIFI;
    }
    if ([MainModel model].networkStatus == AFNetworkReachabilityStatusReachableViaWWAN) {   //蜂窝
        networkType = NETWORK_TYPE_3G;
    }
...
+ (BOOL)isWifiNetworkEnvironment{
    int num = [self dataNetworkTypeFromStatusBar];
    if (num == NETWORK_TYPE_WIFI) {
        return YES;
    }
    return NO;
}
