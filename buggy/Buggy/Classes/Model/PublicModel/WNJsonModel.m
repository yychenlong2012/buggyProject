...
+ (NSDictionary *)dictionaryWithModel:(id)model {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if([propertyValue isEqual:[NSNull null]] || [propertyValue isEqualToString:@"(null)"])
        {
            propertyValue = @"-";
        }
...
//        NSLog(@"Data %@ type: %@",memberName,dataType);
        ...
        for (int j = 0; j < count; j++) {
            ...
            //TODO:这里是判断 model的属性和 NSDictionary的映射规则 
            if (range.location == NSNotFound) {
                continue;
            } if (memberName.length != (propertyName.length + 1)){
                continue;
            } else {
                ...
                if (propertyValue == nil || [propertyValue isKindOfClass:[NSNull class]]) {  //如果propertyValue为nil那么下面会报错崩溃
                    propertyValue = @"";
                }
