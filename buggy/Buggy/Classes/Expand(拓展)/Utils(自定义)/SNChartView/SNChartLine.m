...
#define kBtnTag 100
#define kLineColor [UIColor colorWithRed:1.000f green:1.00f blue:1.00f alpha:1.00f]
#define kCirCleColor [UIColor colorWithRed:0.859f green:0.871f blue:0.882f alpha:1.00f]
#define kHVLineColor [UIColor colorWithRed:0.918f green:0.929f blue:0.949f alpha:0.100f]
#define kBulldesFont [UIFont systemFontOfSize:10]

static const NSInteger kYEqualPaths = 2;//y轴为5等份
static const CGFloat kTopSpace = 26.f;//距离顶部y值
...
- (void)startDrawLines {
    ...
    //设置y轴
#pragma mark - zhengrengen modify 2017.8.2
    for (NSInteger i = 0; i < _xValues.count; i++) {
        [self.pointYArray addObject:@(chartLineTheYAxisSpan * kYEqualPaths - ([_yValues[i] floatValue] == 0 ?0.00001:[_yValues[i] floatValue])/_yMax * chartLineTheYAxisSpan * kYEqualPaths + kTopSpace)];
    }
...
//    btn.layer.borderColor = [UIColor colorWithRed:0.780f green:0.780f blue:0.804f alpha:0.5f].CGColor;
    ...
    [btn addTarget:self action:@selector(doubleClick:) forControlEvents:UIControlEventTouchDownRepeat];
    if (self.points.count == index + 1) {
        [self circleButtonClick:btn];
    }
}
...
- (void)doubleClick:(UIButton *)btn
{
    ...
    if ([self.delegate respondsToSelector:@selector(doubleClickItem:)]) {
        [self.delegate doubleClickItem:btn.tag - kBtnTag];
    }
}
