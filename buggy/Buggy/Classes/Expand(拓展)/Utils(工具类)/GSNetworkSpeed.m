...
- (void)start{
    if (_timer == nil) {
        _timer = [NSTimer scheduledTimerWithTimeInterval:0.25 target:self selector:@selector(checkNetworkSpeed) userInfo:nil repeats:YES];
        [_timer fire];
    }
}
...
- (void)checkNetworkSpeed{
    ...
    if (_iBytes != 0) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(downloadSpeed:)]) {
            
            [self.delegate downloadSpeed:_downloadNetworkSpeed];
        }
//        NSLog(@"_downloadNetworkSpeed : %@",_downloadNetworkSpeed);
    ...
    if (_oBytes != 0) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(uploadSpeed:)]) {
            
            [self.delegate uploadSpeed:_uploadNetworkSpeed];
        }
