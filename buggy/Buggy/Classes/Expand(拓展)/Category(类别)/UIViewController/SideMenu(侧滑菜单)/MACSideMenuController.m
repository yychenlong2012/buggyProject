...
- (instancetype)initWithLayoutSubviewsHandler:(void(^)(void))layoutSubviewsHandler
{
    self = [super init];
    if (self)
    {
...
- (void)hiddensInvalidateWithDelay:(NSTimeInterval)delay
{
    ...
    else if (!self.isLeftViewShowing)
    {
        if (delay)
        {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^(void)
                           {
                               self->_rootViewCoverViewForLeftView.hidden = YES;
                               self->_leftViewCoverView.hidden = YES;
                               self->_leftView.hidden = YES;
                               self->_leftViewStyleView.hidden = YES;
                           });
        }
        else
        {
    ...
    else if (!self.isRightViewShowing)
    {
        if (delay)
        {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^(void)
                           {
                               self->_rootViewCoverViewForRightView.hidden = YES;
                               self->_rightViewCoverView.hidden = YES;
                               self->_rightView.hidden = YES;
                               self->_rightViewStyleView.hidden = YES;
                           });
        }
        else
        {
    ...
    if (rootViewStyleViewHiddenForLeftView && rootViewStyleViewHiddenForRightView)
    {
        if (delay)
        {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^(void)
                           {
                               self->_rootViewStyleView.hidden = YES;
                           });
        }
    ...
    if (backgroundImageViewHiddenForLeftView && backgroundImageViewHiddenForRightView)
    {
        if (delay)
        {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^(void)
                           {
                               self->_backgroundImageView.hidden = YES;
                           });
        }
        ...
        _leftView = [[MACSideMenuView alloc] initWithLayoutSubviewsHandler:^(void)
                     {
                         if (wself)
                         {
                             ...
                             [self leftViewWillLayoutSubviewsWithSize:CGSizeMake(self->_leftViewWidth, size.height)];
                         }
                     }];
        _leftView.backgroundColor = [UIColor clearColor];
        _leftView.hidden = YES;
        ...
        _rightView = [[MACSideMenuView alloc] initWithLayoutSubviewsHandler:^(void)
                      {
                          if (wself)
                          {
                              ...
                              [self rightViewWillLayoutSubviewsWithSize:CGSizeMake(self->_rightViewWidth, size.height)];
                          }
                      }];
        _rightView.backgroundColor = [UIColor clearColor];
        _rightView.hidden = YES;
...
- (void)showLeftViewAnimated:(BOOL)animated fromPercentage:(CGFloat)percentage completionHandler:(void(^)(void))completionHandler
{
    if (animated)
    {
...
- (void)hideLeftViewAnimated:(BOOL)animated fromPercentage:(CGFloat)percentage completionHandler:(void(^)(void))completionHandler
{
...
- (void)showRightViewAnimated:(BOOL)animated fromPercentage:(CGFloat)percentage completionHandler:(void(^)(void))completionHandler
{
    if (animated)
    {
        [MACSideMenuController animateStandardWithDuration:0.5//*(1.f-percentage)
...
- (void)hideRightViewAnimated:(BOOL)animated fromPercentage:(CGFloat)percentage completionHandler:(void(^)(void))completionHandler
{
...
+ (void)animateStandardWithDuration:(NSTimeInterval)duration animations:(void(^)(void))animations completion:(void(^)(BOOL finished))completion
{
