...
{
    [self showAlertMessage:message titile:NSLocalizedString(@"提示",nil)];
...
-(void)showAlertMessage:(NSString*)message titile:(NSString *)title{
    ...
    NSArray *items =
    @[MMItemMake(NSLocalizedString(@"确定",nil), MMItemTypeHighlight, block)];
...
+ (UIViewController *)topViewController {
    ...
    //1、先找排除导航控制器和tabber控制器
    resultVC = [self _topViewController:[window rootViewController]];
    //2、判断当前ViewController是否通过presentViewController:animated:completion:模态展示了其他ViewController
