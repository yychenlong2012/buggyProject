        ...
        if (self.mj_footer==nil) {
            [self setMj_footer:[MJRefreshAutoNormalFooter
                                footerWithRefreshingBlock:^{
                                    // 加载下一页在这
                                    [self showWaiting];
                                    if (self->_macTableViewDelegate && [self->_macTableViewDelegate respondsToSelector:@selector(loadDataRefreshOrPull:)]) {
//                                        if (self.mj_footer.state==MJRefreshStateIdle) {
//                                            return ;
//                                        }
                                      //  MJRefreshState state=self.mj_footer.state;
                                        [self->_macTableViewDelegate loadDataRefreshOrPull:MACPulling];
                                    }
